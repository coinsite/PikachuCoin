...
void ThreadMapPort(void* parg)
{
    ...
    RenameThread("PinkachuCoin-UPnP");
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "PinkachuCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
void ThreadOpenConnections2(void* parg)
{
    ...
    loop
    {
        ...
        if (addrman.size()==0 && (GetTime() - nStart > 60) && !fTestNet)
        {
            ...
            {
                // It'll only connect to one or two seed nodes PKCause once it connects,
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. PinkachuCoin is probably already running."), addrBind.ToString().c_str());
        else
