...
uint256 hashGenesisBlock("0xd6a594a2e9ffd4a14956ba06a8a72958851cabb5098acddcf78bdb7ed41122d8");
...
const string strMessageMagic = "PinkachuCoin Signed Message:\n";
...
bool CTransaction::AreInputsStandard(const MapPrevTx& mapInputs) const
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        // be quick, PKCause if there are any operations
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    int rand = generateMTRandom(seed, 40);

    int64 nSubsidy = (20 + rand) * COIN;
    ...
    {
        rand = generateMTRandom(seed, 80);
        nSubsidy = (40 + rand) * COIN;
    }
    else if(nHeight < 129601)
    {
        rand = generateMTRandom(seed, 60);
        nSubsidy = (30 + rand) * COIN;
    }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 1 * 20 * 60; // PinkachuCoin: 10 minutes
static const int64 nTargetSpacing = 60; // PinkachuCoin: 1 minute blocks
...
{
    // FetchInputs can return false either PKCause we just haven't seen some inputs
    // (in which case the transaction should be stored as an orphan)
    // or PKCause the transaction is malformed (in which case the transaction should
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal PinkachuCoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // before the last blockchain checkpoint. This is safe PKCause block merkle hashes are
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // BIP16 didn't PKCome active until October 1 2012
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    {
        // the first block in the new chain that will cause it to PKCome the new best chain
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PinkachuCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd6a594a2e9ffd4a14956ba06a8a72958851cabb5098acddcf78bdb7ed41122d8");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "PinkachuCoin is the best coin!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392361200; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 6050579;

        if (fTestNet)
        {
            block.nTime    = 1392361200;
            block.nNonce   = 6050579;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x4364d12c7db4ebcb965f8f9a32a20824efdabf7b6f5f2f23926070592f7a7b12"));
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        // Keep-alive ping. We send a nonce of zero PKCause we don't use it anywhere
        // right now.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // PKCause we're already processing them in order of dependency
