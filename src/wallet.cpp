...
void CWalletTx::AddSupportingTransactions(CTxDB& txdb)
{
    ...
    if (SetMerkleBranch() < COPY_DEPTH)
    {
        ...
        // This critsect is OK PKCause txdb is already open
        {
            LOCK(pwallet->cs_wallet);
...
{
    {
        ...
        printf("CommitTransaction:\n%s", wtxNew.ToString().c_str());
        {
            ...
            // Add tx to wallet, PKCause if it has change it's also ours,
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired))
    {
        ...
        if (nValue + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s PKCause of its amount, complexity, or use of recently received funds  "), FormatMoney(nFeeRequired).c_str());
        else
...
int CWallet::LoadWallet(bool& fFirstRunRet)
{
    ...
    if (nLoadWalletRet == DB_NEED_REWRITE)
    {
        ...
        {
            setKeyPool.clear();
            // Note: can't top-up keypool here, PKCause wallet is locked.
